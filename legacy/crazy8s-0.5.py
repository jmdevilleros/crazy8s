...
from cards import FrenchDeck # uses v0.4 or v0.5
import os
import time
import random
...
class Crazy8s():
    ...
    def play(self, display=True, delay=0, pause=True):
        ...
        firstplayer_seq = random.randint(1, numplayers)
        player_seq = firstplayer_seq
        while self.winner == None:
            if player_seq == firstplayer_seq:
                self.turn += 1
            self.currentplayer = self.players[player_seq - 1]
            self.currentplayer.hand.flip()
            self.displaystate(display, delay, pause)
            self.playerturn(self.currentplayer)
            self.currentplayer.hand.flip()
            if len(self.currentplayer.hand.cards) < 1:
                self.winner = self.currentplayer
            else:
                player_seq = (player_seq % numplayers) + 1
        return self.winner
