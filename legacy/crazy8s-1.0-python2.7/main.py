...
class Player:
    ...
    def createlist(numplayers, prefix="Player"):
        ...
        fmt = '{:0' + str(seq_len) + '}' 
        for seq in range(1, int(numplayers + 1)):
...
class Card:
    ...
    def __repr__(self):
        return str(self.symbol
                   if not self.facedown else self.fdsymbol) + ' '
...
class CardStack:
    ...
    # ==================================================================
    def __init__(self, cards=list(), shuffle=False, facedown=None):
    ...
    def __contains__(self, item):
        ...
        return (item in self.suits()) or \
               (item in self.ranks()) or \
               (item in self.cards)
    ...
    # ==================================================================
    def search(self, ranks=list(), suits=list(),
               cards=list(), negate=False):
...
class FrenchDeck:
    ...
    def create(shuffle=False, facedown=False):
        ...
        cards = [
            Card(suit, rank, '')  # chr(0X1F0A1 + 16 * s + r + r // 11))
    ...
    def loadimages(card_stack, dirname='./', ext='.png', back='back'):
        ...
        back_image = dirname + back + ext
        for card in card_stack:
            filename = card.name.lower().replace(' ', '_')
            front_image = dirname + filename + ext
            card.front = front_image
...
class GameScreen(Screen):
    ...
    def update_card_zone(zone, cards):
        ...
        for card in cards:
            ...
            zone.add_widget(Image(source=image))
